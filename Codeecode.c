...
{   
    ...
    "Ve do thi ham so trong khoang (a,b)",
...
double f1(double x);
double f2(double x);
double min_val(double f(double x), double a, double b);
double max_val(double f(double x), double a, double b);
...
// Prototypes cho cac ham ve do thi
void write_data_to_file(const char *filename, double a, double b, int steps);
void write_gnuplot_script(const char *script_name, const char *data_file, double a, double b);

// Prototypes cho cac ham trong giao_dien.h (neu can)
void khung();
void khungngoai();
void khungthongtin();
void khunghuongdan();
void khungtieptuyen();
void khungexit();
void arrowHere(int i, int position);
...
bool check_input(double a, double b){
    ...
    else if (sign(max_val(df, a, b)) != sign(min_val(df, a, b))) return false;
    else if (sign(max_val(ddf, a, b)) != sign(min_val(ddf, a, b))) return false;
    else return true;
}
...
// Ham thuc hien chuc nang 3: Ve do thi bang gnuplot
void write_data_to_file(const char *filename, double a, double b, int steps) {
...
void chucnang_3() {
    ...
    const char *script_file = "plot_script.gp";
    int steps = 100;
    double a, b;  
    ...
    TextColor(113);
    gotoxy(16, 9); printf("3. Ve do thi ham so f(x) trong khoang (a,b)");
    gotoxy(16, 11); printf("%c Nhap can duoi a: ", 175);
    scanf("%lf", &a);
    gotoxy(16, 12); printf("%c Nhap can tren b: ", 175);
    scanf("%lf", &b);

    gotoxy(84, 3); printf("Ve do thi ham so f(x) trong khoang [%.6lf, %.6lf]", a, b);
    write_data_to_file(data_file, a, b, steps);
    write_gnuplot_script(script_file, data_file, a, b);

    int ret = system("gnuplot plot_script.gp");
    if (ret != 0) {
        gotoxy(84, 5); printf("Loi khi goi gnuplot. Vui long kiem tra da cai gnuplot.");
    }
}
...
void chucnang_4()
{
    ...
    fprintf(file, "\nChuc nang 4: Tim nghiem gan dung voi so lan lap cho truoc trong khoang (a, b) va danh gia sai so epsilon theo hai cong thuc sai so.");
    print_poly_to_file(file);
...
void chucnang_5()
{
    ...
    fprintf(file, "\nChuc nang 5: Tim nghiem gan dung trong khoang (a, b) voi sai so epsilon cho truoc.");
    print_poly_to_file(file);
...
// gotoxy và TextColor đã được định nghĩa trong giao_dien.h
void menu() 
{   
    int position = 1;
    int keyPress = 0;
